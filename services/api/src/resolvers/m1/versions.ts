...
export async function approveDocumentVersion(event: AppSyncEvent, tenantId: string, actor: string) {
...
export async function listDocumentVersions(event: AppSyncEvent, tenantId: string) {
  const txn = await beginTenantTransaction(tenantId);
  try {
    const result = await txn.execute(
      `SELECT * FROM m1.document_versions WHERE document_id = :documentId::uuid ORDER BY version_no DESC LIMIT ${LIST_QUERY_LIMIT}`,
...
export async function getDocumentVersionDiff(event: AppSyncEvent, tenantId: string) {
  const txn = await beginTenantTransaction(tenantId);
  try {
...
export async function getDocumentContent(event: AppSyncEvent, tenantId: string) {
  const txn = await beginTenantTransaction(tenantId);
  try {
    const result = await txn.execute(
...
/**
 * saveDocumentSectionEdit (ES-4) — a human edit lands as a NEW version (same
 * 7.5.2 versioning law as everywhere else; content edits never mutate in
